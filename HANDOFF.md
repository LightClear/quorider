...
- `test/engine.test.js`（32 项）：走子/跳跃/斜走、墙的占用与交叉、封死判定、中央获胜、
  ...
  踩中陷阱跳过一回合且自己埋的雷不炸自己。
- `test/rooms.test.js`（17 项）：房间管理层的确定性单测——不限时房间「被陷阱困住必须
  自动跳过」的防卡死兜底、限时房间超时托管走的是 skip、逐人状态的隐私过滤
